...
const MentorClassDetails = () => {
  ...
  useEffect(() => {
    const getClassDetails = async (userId: string) => {
      try {
        const data = await fetchClassDetails(userId, "mentorId");
        setClasses(data);
      } catch (error) {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 pt-6">
      {classes.map((classItem: any) => (
        <Card key={classItem._id} className="border-violet-500 dark:bg-violet-500">
          <CardHeader>
            <CardTitle className="text-violet-500 dark:text-white">
              {classItem.subjectName}
            </CardTitle>
            <CardDescription className="dark:text-slate-100">{classItem.classNumber}</CardDescription>
          </CardHeader>
          <CardContent>
...
export default MentorClassDetails;
